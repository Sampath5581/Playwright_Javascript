...
test.describe('Recurring Charges Forms', () => {
    ...
    test.beforeEach(async () => {
        ...
        try {
            if (currentUrl.includes(process.env.REC_DASHBOARD_URL)) {
                await test.step('Verify whether User is able to Navigate to recurring-charges-form Page', async () => {
                ...
                await test.step('Verify whether User is able to Navigate to recurring-charges-form Page', async () => {
                    ...
                    await recurring_forms.click_on_selected_form('Recurring Charges');
                    await expect(page).toHaveURL("https://pulse-test.cbre.com/Forms/recurring-charges-form");
                });
            }
        } catch (error) {
            ...
            if (process.env.ENV === 'REC') {
                ...
                const storageStatePath = process.env.ENV === 'REC'
                    ? path.resolve(path.join(__dirname, '../', process.env.TEST_RESULTS_DIR), `${random_creator}_state.json`)
                    : path.resolve(path.join(__dirname, '../', process.env.TEST_RESULTS_DIR), `${random_creator}_state.json`);
                //const storageStatePath = process.env.ENV === 'REC' ? path.resolve(__dirname, '../../test-results/REC_ADMIN_state.json') : path.resolve(__dirname, '../../test-results/TEST_ADMIN_state.json');
                const storageState = JSON.parse(fs.readFileSync(storageStatePath, 'utf8'));
                context = await browser.newContext({ storageState });
                page = await context.newPage();
            }
    ...
    test('2307403 - Create a Recurring Charges Billing Form with Positive total amount and with out attachments using random_creator Role and approve it with random_approver role', async ({ browser }) => {
        ...
        await test.step(`2307404 - Verify whether User is able to Approve the Recurring Charges Form with (Primary Approver ${random_approver} when amt value is positive)`, async () => {
            context = await browser.newContext({ storageState: `${random_approver}` + "_state.json" });
            page = await context.newPage();
    ...
    test("2307421 - Verify that the secondary approver can't approve if it is not approved by Primary approver", async ({ browser }) => {
        ...
        await test.step(`2307421 - Verify that the secondary approver ${random_secondary_approver} can't approve if it is not approved by Primary approver ${random_approver})`, async () => {
            context = await browser.newContext({ storageState: `${random_secondary_approver}` + "_state.json" });
            page = await context.newPage();
            await page.goto(process.env.REC_URL);
            await page.waitForTimeout(5000);
            const currentUrl = page.url();
